...
class AgentManager:
    ...
    async def run_agent_method(
        ...
        if not hasattr(component, method_name):
            raise AttributeError(
                ...
                the component has the following attributes: {dir(component)}"
            )
    ...
    def _generate_single_agent_config(
        ...
        if not template_name:
            if not self._agent_templates.templates:
                logger.error("No templates available for dynamic agent creation.")
                return None
            template = self._agent_templates.templates[0]
            logger.info(f"No template name provided, using default template '{template.name}'.")
        else:
